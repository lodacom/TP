...
to setup
  ...
  set-default-shape abeilles "wolf"
  ...
  set-default-shape reines "rabbit"
end
...
to agiter-abeilles [n]
  ifelse one-of reines in-radius n = nobody
  [
    agiter 
  ]
  [
    ;;set heading towards one-of reines in-radius n
    let p max-one-of neighbors [odeur]  ;; or neighbors4
...
to diffusion
  diffuse odeur taux-diffusion / 100;;diffusion de l'odeur sur une variable appartenant aux patches
end
...
to go
  ...
  ask patches 
  [
    set pcolor scale-color yellow odeur 1 (depot-odeur / 1.3);;couleur allant du jaune au blanc 
    set odeur odeur - taux-evaporation / 100;;on fait évaporer l'odeur
    ] 
  tick
end
...
to go-reine
 agiter-reines
 set odeur odeur + depot-odeur;;on ajoute l'odeur courante du patche au depot-odeur du lapin
end
