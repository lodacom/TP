...
to search-for-chip  ;; turtle procedure -- "picks up chip" by turning orange
  ...
  [ ;;face patch 0 0  rassemble au centre
    set pcolor black ;;transformation du terrain en noir
    set color orange ;;je change ma couleur en orange
    fd 20 ]
  [ ifelse pcolor = green
    [
      set pcolor black
...
to find-new-pile  ;; turtle procedure -- look for yellow patches
  ifelse pcolor != yellow and color = orange ;;si le patche n'est pas jaune et que je suis orange
  [ ;;si je rencontre un patche(terrain) jaune il faut que je soit orange pour pauser (sinon je continu à chercher)
    wiggle
    find-new-pile
  ]
  [
    if pcolor != green and color = blue
    [
      wiggle
      find-new-pile
    ]
...
to put-down-chip  ;; turtle procedure -- finds empty spot & drops chip
  ifelse pcolor = black and color = orange
  [ ;; le terrain et noir et je suis orange (auparavant j'ai trouvé la pile qui me convenait)
    set pcolor yellow ;;le terrain devient jaune
    set color white ;; je devient blanc
    get-away ]
  [ 
    ifelse pcolor = black and color = blue
    [
      set pcolor green
      set color white
    get-away ]
